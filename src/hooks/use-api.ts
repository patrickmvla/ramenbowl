...
import { year, getNextSeason, getCurrentSeason } from "../index";
...
const axiosInstance = axios.create({
  ...
  timeout: 10000,
  headers: {
    "X-API-Key": API_KEY, // Assuming your API expects a key in this header
  },
});
...
// Function to create with given cache key
...
// Individual caches for different type of data
// Creating cache for anime data, anime info, and video sources
...
// Function to fetch data proxy with caching
async function fetchFromProxy(url: string, cache: any, cacheKey: string) {
  try {
    ...
    const requestConfig = PROXY_URL
      ? { params: { url } } // If PROXY_URL is defined, send the original URL parameter
...
export async function fetchAdvancedSearch(
  ...
  const queryParams = new URLSearchParams({
    ...
    ...(options.sort && { sort: JSON.stringify(options.status) }),
  });

  if (options.genres && options.genres.length > 0) {
    // Correctly encode genres as string array
...
// Function to fetch list of anime base on type (TopRated, Trending, Popular)
async function fetchList(
  ...
  if (
    ...
    if (type === "TopRated") {
      ...
      url = `${BASE_URL}meta/anilist/advanced-search?type=${options.type}&statue=${options.status}&sort=${options.sort}&season=${options.season}&year=${options.year}&`;
...
// Fetch embedded anime episode servers
...
// Function to fetch skip times for an episode
